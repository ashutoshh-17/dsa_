...
/*Algo:
1. Insert Operation (void BST::insert()):
Prompt the user for the data to be inserted in the new node.
Create a new node with the given data.
If the tree is empty, set the new node as the root.
Traverse the tree to find the appropriate position for the new node based on the BST property.
Repeat the process until an empty spot is found, and insert the new node.

3. Display Operation (void BST::display(Node* root)):
Perform an in-order traversal of the tree (left subtree, root, right subtree).
Display the data of each node.
4. Mirror Operation (void BST::mirror_tree(Node* t)):
Recursively traverse the tree.
Swap the left and right subtrees of each node.
